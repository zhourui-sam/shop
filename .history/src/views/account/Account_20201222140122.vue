  ...
  <div>
    ...
    <div class='shopping'>
      ...
      <div v-if="this.flag==1">
        <van-submit-bar :price="total*100" button-text="提交订单" @submit="onSubmit1" />
      </div>
      <div v-else>
        <van-submit-bar :price="total*100" button-text="提交订单" @submit="onSubmit" />
      </div>
...
export default {
  ...
  methods: {
      ...
      getId(){
        // this.GoodsId1.push(JSON.parse(this.$route.query.goods)[0].id)
        this.GoodsId1.push(this.goods.id)
        // this.count1= JSON.parse(this.$route.query.goods)[0].count
      },
      GoAddress(){
  ...
  mounted() {
    console.log(JSON.parse(this.$route.query.goods)[0])
      this.getId()
      this.GetAddress()
      this.GetGoods()
  },
