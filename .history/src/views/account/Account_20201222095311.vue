    ...
    <div v-if="flag === 1">
      <div class='goods'>
        ...
        <div class='goods-detail'>
          <div class='goods-name'>{{comebuy.name}}</div>
          <div class='goods-price'>￥{{comebuy.present_price}}</div>
        </div>
        <div class='goods-count'>x{{comebuy.count}}</div>
      </div>
      <div style="height:50px"></div>
      <van-submit-bar :price="total*100" button-text="提交订单" @submit="buySubmit" />
    </div>
...
export default {
  ...
  methods: {
      ...
      GetGood(){
        this.goods = JSON.parse(this.$route.query)
      },
      onSubmit(){
        this.$api.OrderData({
      ...
      buySubmit(){
        this.$api.OrderData({
          address : this.defaultadd.address,
          tel : this.defaultadd.tel,
          orderId : this.comebuy.goods.id,
          totalPrice : this.comebuy.value*this.comebuy.goods.present_price,
          idDirect : true,
          count : this.comebuy.value
        }).then(res =>{
          Toast.success(res.msg)
          this.$router.push('/cards')
        })
  ...
  mounted() {
      this.GetAddress()
      this.GetGoods()
      this.comebuy = JSON.parse(this.$route.query)
