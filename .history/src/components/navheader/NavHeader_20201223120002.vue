<template>
  <div class='contain'>
    ...
    <div class='position' @click="AddPosition">{{address}}<van-icon name="arrow-down"/></div>
      ...
      <van-popup v-model="show" position="bottom" :style="{ height: '94%' }" :overlay='false' closeable close-icon="close">
        <div v-for="(item ,index) in this.list" :key="index">
          <div class="box" v-html="item.name.replace(value, `<span style='color:red'>${value}</span>`)"></div>
        </div>
      </van-popup>
  </div>
</template>
