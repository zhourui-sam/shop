<template>
  <div class='bg'>
    ...
    <!--父组件给子组件传数据，需要用v-if判断，才不会刷新页面数据获取不到，数据类型为对象形式时需要用Objext.keys(数据)的形式判断对象的长度是否大于0-->
      ...
      <mainpicture :goods='goods' :comment='comment' v-if="Object.keys(goods).length>0"></mainpicture>
    </div>
    ...
    <div class='bottom'>
      <van-goods-action>
        ...
        <van-goods-action-button type="danger" text="立即购买" @click="buyItem"/>
...
export default {
  ...
  methods: {
    ...
    buyItem(){
      this.$checkLogin(this.buydetail)
    },
    gobuy(goods){
